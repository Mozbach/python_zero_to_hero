...
def has_33(numList) :
    for i in range(0 , len(numList) -1) :
        if numList[i : + 2 ] == [3, 3] : # [i + i + 2] means FROM current iteration to current iteration + 2. Remember, [Start:End:Skip]
            return True
    return False
...
# Step 1: Create a function named black_jack, taking 3 integer arguments
# Step 2: Create 2 variables, one fro totalSum and one holding the args -> As to not re-write this the whole time
# Step 3: Using an if-else queue and the two variables, return as instructed -> Pretty easy, but according to Lourenzo, there is a better way.
def black_jack(a, b, c) :
    arg_total = sum((a, b, c))
    args = (a, b, c)
    if arg_total <= 21 : 
        return arg_total
    elif arg_total > 21 and 11 in args :
        return arg_total - 10
    elif arg_total > 21 and 11 not in args :
        return 'BUSTED!'
    else :
        pass
